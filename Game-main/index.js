...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Make logger easily accessible from console
    window.logger = window.game.logger;
    
    // Add convenient console functions
    window.viewStats = () => {
        const stats = window.game.logger.displayStats();
        console.table(stats);
        return stats;
    };
    
    window.exportGameLog = () => {
        window.game.logger.exportLog();
        console.log('📥 Log file downloaded!');
    };
    
    window.clearGameLogs = () => {
        if (confirm('Are you sure you want to clear all game logs?')) {
            window.game.logger.clearLogs();
    ...
    window.viewRawLogs = () => {
        ...
        console.log('📊 Raw game logs:', logs);
        return logs;
    };
    
    // NEW: Date and time organized viewing functions
    window.viewGamesByDate = () => {
        return window.game.logger.displayGamesByDate();
    };
    
    window.viewRecentGames = () => {
        return window.game.logger.displayRecentGames();
    };
    
    window.viewGamesForDate = (date) => {
        if (!date) {
            console.log('📅 Usage: viewGamesForDate("12/27/2024")');
            console.log('📅 Available dates:');
            const logs = JSON.parse(localStorage.getItem('ticTacToeGameLogs') || '[]');
            const dates = [...new Set(logs.map(game => game.date))].sort();
            dates.forEach(d => console.log(`   • ${d}`));
            return;
        }
        return window.game.logger.displayGamesForDate(date);
    };
    
    window.viewDetailedHistory = () => {
        return window.game.logger.displayDetailedHistory();
    };
    
    // Log helpful console commands
    console.log(`
🎮 Tic Tac Toe Logging System Ready!

📊 Basic Commands:
• viewStats()           - View current statistics
• exportGameLog()       - Download log file
• clearGameLogs()       - Clear all logs

📅 Date & Time Views:
• viewGamesByDate()     - Games organized by date
• viewRecentGames()     - Last 10 games played
• viewGamesForDate("12/27/2024") - Games for specific date
• viewDetailedHistory() - Complete detailed history

📋 Advanced Access:
• game.logger          - Access logger directly
• logger               - Shortcut to logger
• viewRawLogs()        - See raw game data
    `);
});
